import { getSheets, TAB, HEADERS, projectToRow, requireEditKey, parseBody } from "./_lib/sheets.js";

/* The 8 one-pagers as of June 2026 — the initial portfolio. */
const SEED = [
  {
    id: "ms-01",
    code: "MS-01",
    title: "Multi-Touch Attribution Model",
    workstream: "Marketing Services",
    stakeholder: "Marketing Services (Paid Media)",
    revopsRole: "Project coordinator + systems build",
    devResources: "Data team",
    contractors: [],
    teams: ["Marketing Services", "RevOps", "Data"],
    problem:
      "Moxie lacks a unified attribution model linking paid advertising (Meta/Google) to revenue. Current pipelines only track Meta form submissions, missing primary Google Ads drivers like phone calls. We cannot differentiate organic vs. paid traffic or view cross-channel patient journeys, preventing data-backed marketing decisions.",
    solution:
      "Build a multi-touch attribution architecture recording every interaction (forms, calls, visits, referrals) as a timestamped touchpoint, rolling up to patient bookings for cross-channel reporting. After a Data-led build-vs-buy evaluation, integrate attribution data into Omni, Practice Accelerator, HubSpot, and ClickUp.",
    deliverables: [
    ...
    success:
      "Accurate, actionable revenue reporting per practice (including phone calls). The model deduplicates touches and distinguishes organic from paid, with visibility in Omni and Practice Accelerator.",
    dependsOn: [],
    openItems: ["No baseline metric or target defined", "No effort estimate in source doc"],
    impact: 5,
    effort: 5,
    status: "Scoping",
  ...
  },
  {
    ...
    success:
      "ClickUp v1 workflows live as interim; selected platform live with per-client separation before end of year. Materially reduced manual ad configuration; platform used for spend optimization decisions.",
  ...
  },
  {
    id: "ms-03",
    code: "MS-03",
    title: "AI-Powered Creative Production Workflow",
    workstream: "Marketing Services",
    stakeholder: "Marketing Services (Paid Media)",
    revopsRole: "Project coordinator + business systems build",
    devResources: "Product (AI PM)",
    ...
    success:
      "Creative team produces more differentiated ad creatives across all paid channels in meaningfully less time, with reduced manual file handling — on a foundation that can absorb attribution and media platform inputs as those mature.",
    dependsOn: [
  ...
  },
  {
    id: "ms-04",
    code: "MS-04",
    title: "Meta Ads Segment-Based Service Gating",
    workstream: "Marketing Services",
    stakeholder: "Marketing Services (Paid Media)",
    revopsRole: "Project coordinator + business systems build",
    devResources: "N/A",
    ...
    teams: ["Marketing Services", "RevOps"],
    problem:
      "Meta Ads service eligibility rules exist in policy but are not enforced by systems, allowing customers and PSMs to submit requests for services their segment doesn\u2019t qualify for. This creates invalid campaign requests, internal escalations, and inconsistent service delivery.",
    solution:
      "Define a segment-to-service eligibility matrix for Meta Ads aligned to the new segmentation framework (Growth, Silver, Gold, Platinum, Diamond, VIP), then enforce it via HubSpot form access restrictions gated by segment field and ClickUp workflow warnings on related campaign tasks.",
    deliverables: [
    ...
    success:
      "Customers and PSMs are technically unable to submit Meta Ads requests outside their segment\u2019s eligibility — invalid requests are blocked or flagged at the system level before reaching the Meta Ads team.",
    dependsOn: [],
    openItems: ["ClickUp contractor unsourced", "Assumes segment field is populated and current in HubSpot"],
    impact: 3,
    effort: 2,
    status: "Scoping",
  ...
  },
  {
    id: "sup-01",
    code: "SUP-01",
    title: "Account Setup & Visibility",
    workstream: "Supplies",
    stakeholder: "Supplies team (Shannon Aubert, OAs)",
    revopsRole: "Project coordinator + business systems build",
    ...
    problem:
      "Account setup and migration runs across HubSpot, Zapier, and a Google Sheet, with no real-time status visibility outside the Supplies team. Providers and PSMs repeatedly ask via Slack and email where an account stands, slowing onboarding and pulling OAs away from setup work. As provider volume grows, this doesn\u2019t scale without headcount.",
    solution:
      "Migrate the full account setup intake and task lifecycle into ClickUp, with status mirrored into a new HubSpot Supplies Pipeline (backed by a Supplies Accounts custom object). Automated Intercom tickets notify Providers of key milestones so they — and their PSMs — self-serve status.",
    deliverables: [
    ...
    success:
      "PSMs and Providers find setup status without contacting the Supplies team — Slack status inquiries drop materially within 60 days of launch. OAs work entirely out of ClickUp with no Google Sheets reliance.",
    dependsOn: [],
    openItems: ["ClickUp contractor unsourced", "Slack-inquiry baseline not yet measured"],
    impact: 4,
    effort: 3,
    status: "Scoping",
  ...
  },
  {
    id: "sup-02",
    code: "SUP-02",
    title: "Ordering Process Improvements",
    workstream: "Supplies",
    stakeholder: "Supplies team (Shannon Aubert, OAs)",
    revopsRole: "Project coordinator + business systems build",
    ...
    success:
      "OAs no longer manually log into vendor portals for routine orders; orders no longer dropped or mis-placed for lack of task structure. Fulfillment status checked from a single dashboard rather than per-vendor.",
  ...
  },
  {
    id: "sup-03",
    code: "SUP-03",
    title: "Spend & Rebate Reporting",
    workstream: "Supplies",
    stakeholder: "Supplies team (Shannon Aubert, OAs)",
    revopsRole: "Project coordinator + business systems build",
    devResources: "Data team (Snowflake, Omni)",
    contractors: [
    ...
    problem:
      "Supplies orders run through a unified Shopify portal, but spend data isn\u2019t connected to rebate targets in any systematic way. No real-time view of spend vs. rebate thresholds, rebate commitments aren\u2019t stored anywhere structured, and spend isn\u2019t tied back to HubSpot for PSM account reviews.",
    solution:
      "Pull Shopify order data and rebate targets into the Data Warehouse and surface them in an Omni dashboard showing spend progress against thresholds. When spend nears a rebate level, a ClickUp workflow triggers a review to update the Shopify catalog and push over the threshold. Key spend metrics sync back to HubSpot.",
    deliverables: [
    ...
    success:
      "Supplies team sees in real time where spend stands vs. each rebate threshold, with review tasks surfaced automatically. PSMs reference a provider\u2019s spend history directly from HubSpot without contacting the Supplies team.",
  ...
  },
  {
    id: "ps-01",
    code: "PS-01",
    title: "Increase Tier 0 Visibility in Moxie Suite",
    workstream: "Practice Success",
    stakeholder: "Practice Success",
    revopsRole: "Project coordinator + technical consultant",
    devResources: "Product",
    contractors: [],
    teams: ["Practice Success", "RevOps", "Product"],
    problem:
      "Moxie Concierge is hard for providers to discover and accessible from only limited areas of Moxie Suite, so providers bypass it and contact their PSM directly for questions Concierge could resolve. Those who find it often don\u2019t know what it can handle, requiring trial and error before they find value.",
    solution:
      "Work with Product to increase Concierge visibility and accessibility across more Moxie Suite pages, making it an obvious resource at any point in a provider\u2019s workflow. Stretch: pre-load Concierge with page-level context at time of access so it\u2019s ready for questions relevant to where the provider is.",
    deliverables: [
    ...
    success:
      "Provider adoption of Concierge increases meaningfully by usage volume — rollout gated on sufficient Tier 1 support capacity to absorb the expected increase in escalations.",
...
export default async function handler(req, res) {
  try {
    if (req.method !== "POST") {
      return res.status(405).json({ error: "POST only. Example: curl -X POST https://<your-app>.vercel.app/api/seed -H 'x-edit-key: <key>'" });
    }
    if (!requireEditKey(req, res)) return;
    const body = parseBody(req);
    const { sheets, sheetId } = getSheets();

    // Ensure the "Projects" tab exists
    ...
    // Refuse to clobber existing data unless force: true
    const current = await sheets.spreadsheets.values.get({ spreadsheetId: sheetId, range: `${TAB}!A1:A2` });
    const hasData = (current.data.values || []).length > 1;
    if (hasData && !body.force) {
      return res.status(409).json({ error: "Sheet already has data. POST with {\"force\": true} to overwrite." });
    }
    ...
    await sheets.spreadsheets.values.update({
      ...
      requestBody: { values: [HEADERS, ...SEED.map(projectToRow)] },
    });
